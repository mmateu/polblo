...
class CardCRUDTest extends TestCase
{
    ...
    {
        $userBoard = factory(\App\Board::class)-> create(['user_id' => $this->user->id]);  
        $testedBoardCards = factory(\App\Card::class)-> create(['board_id' => $userBoard->id]);
        
        $this->get('board/'.$userBoard->id.'/card')->assertJson(['data' => [$testedBoardCards->toArray()]]);
    }

    public function testUserCanAddCard()
    {
        $userBoard = factory(\App\Board::class)-> create(['user_id' => $this->user->id]);  
        $card = factory(\App\Card::class)-> make(['board_id' => $userBoard->id]); 
        
        $res = $this->post('board/'.$userBoard->id."/card", $card->toArray());
    ...
    {
        $board1 = factory(\App\Board::class)->create(['user_id' => $this->user->id]);
        $board2 = factory(\App\Board::class)->create(['user_id' => $this->user2->id]);

        $card1 = factory(\App\Card::class)->make(['board_id' => $board1->id, 'name'=> $this->exampleName]);
        $card2 = factory(\App\Card::class)->make(['board_id' => $board2->id, 'name'=> $this->exampleName]);
        
        $this->post('board/'.$board1->id."/card", $card1->toArray());
        $this->actingAs($this->user2)->post('board/'.$board2->id."/card", $card2->toArray())->
        assertStatus(200);
    ...
    {
        $board1 = factory(\App\Board::class, 2)->create(['user_id' => $this->user->id]);
        ...
        foreach ($boards as $board)
        {
            ...
            $cards[$board->id] = $card;
            $this->post('board/'.$board->id."/card", $card->toArray())->assertOk();
        }
    ...
    {
        $board = factory(\App\Board::class)->create(['name'=>$this->exampleName,'user_id' => $this->user->id]);       
        $cardTemplate = factory(\App\Card::class)->make(['name'=>$this->exampleName,'board_id' => $board->id]);
